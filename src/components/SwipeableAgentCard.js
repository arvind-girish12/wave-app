...
export default function SwipeableAgentCard({ agent, onSwipeLeft, onSwipeRight, onCTAClick, isTop, animationProps, cardClassName }) {
  ...
  return (
    <>
          ...
          >
            ...
            <div className="w-full mt-4">
              ...
              <div className="flex justify-center mt-2">
                <button
                  className="text-xs underline text-indigo-600 hover:text-indigo-800 transition-colors"
                  onClick={() => setIsChatOpen(true)}
                  tabIndex={0}
                  type="button"
                >
                  Chat with {agent?.name || "AI"}
                </button>
              </div>
              {/* Mobile Share Button */}
              <button
      ...
      {/* Chat overlay (global slide-in) */}
      <ChatWithLLM open={isChatOpen} onClose={() => setIsChatOpen(false)} character={agent} />
    </>
  );
}